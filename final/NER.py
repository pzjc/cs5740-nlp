...
class NERModel(LanguageModel):
  ...
  def load_data(self, debug=False):
    ...
    docs = du.load_dataset('data/ner/train.txt')
    ...
    docs = du.load_dataset('data/ner/dev.txt')
    ...
    docs = du.load_dataset('data/ner/test.masked2')
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope('Output_Layer', initializer=xavier_weight_init()) as scope:
...
def save_predictions(predictions, filename):
  ...
  with open(filename, "wb") as f:
    for prediction in predictions:
      f.write(str(num_to_tag.get(prediction)) + "\n")
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as session:
      ...
      for epoch in xrange(config.max_epochs):
        ...
        save_predictions(predictions, "dev_predicted")
